...
export function getInRangeOffsets(origin, range, opts = {}) {
    ...
    const selfKeys = new Set(originOffsets.map(o => `${o.col},${o.row}`));

    const candidates = new Set();
    if (isHexGrid()) {
        for (const o of originOffsets) {
            const cube = offsetToCube(o.col, o.row);
            for (const h of getHexesInRange(cube, range)) {
                const off = cubeToOffset(h);
                candidates.add(`${off.col},${off.row}`);
            }
        }
    } else {
        for (const o of originOffsets) {
            for (let dx = -range; dx <= range; dx++) {
                for (let dy = -range; dy <= range; dy++) {
                    if (Math.max(Math.abs(dx), Math.abs(dy)) <= range)
                        candidates.add(`${o.col + dx},${o.row + dy}`);
                }
            }
        }
    }
