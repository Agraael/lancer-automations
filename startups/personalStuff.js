...
// Auto-configure token height for vehicles/squads on token creation.
// Hooks into preCreateToken so every new token gets the right height immediately.

function getTokenHeight(actor) {
    ...
    const isVehicle = actor.type === 'npc' && actor.items?.some(i =>
        i.type === 'npc_template' && i.system?.lid === 'npct_vehicle'
    );
    ...
    if (isVehicle) {
        if (size > 4)
            ...
            height = Math.max(0.5, size - 1);
    }

    const isSquad = actor.type === 'npc' && actor.items?.some(i =>
        i.type === 'npc_class' && i.system?.lid === 'npcc_squad'
    );
    if (isSquad)
        height = 0.5;
...
// Apply on token creation
Hooks.on('preCreateToken', (tokenDoc, data) => {
    const actor = tokenDoc.actor;
    if (!actor)
        return;

    const height = getTokenHeight(actor);
    if (height === null)
        return;

    const updates = {};

    // wall-height for prototype tokens, elevatedvision for placed tokens
    updates['flags.wall-height.tokenHeight'] = height;
    updates['flags.elevatedvision.tokenHeight'] = height;

    // Sensor range as sight
    if (actor.system.sensor_range) {
        updates['sight.range'] = actor.system.sensor_range + 5;
        updates['detectionModes'] = [
            { id: "senseAll", enabled: true, range: actor.system.sensor_range },
            { id: "basicSight", enabled: true, range: actor.system.sensor_range + 5 },
            { id: "lightPerception", enabled: true, range: actor.system.sensor_range + 5 }
        ];
    }

    // Dim light based on size
    const size = actor.system.size || 1;
    updates['light.dim'] = size >= 4 ? 2 : 1;
    updates['light.bright'] = size >= 4 ? 2 : 1;
    updates['light.color'] = "#000000";
    updates['light.alpha'] = 0.5;

    // Disable faction border for large NPCs
    if (actor.type === 'npc' && size > 4) {
        updates['flags.token-factions.disableBorder'] = true;
    }
